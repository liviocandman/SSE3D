...
describe('PlanetInfo', () => {
  it('renders standard planet info correctly', () => {
    ...
    expect(screen.getByText('Distance from Earth')).toBeInTheDocument();
  });
  ...
  it('renders moon specific info correctly', () => {
    ...
    // Moon specific stats
    expect(screen.getByText('Distance to Jupiter')).toBeInTheDocument();
    expect(screen.getByText('670,900')).toBeInTheDocument(); // Formatted distance
  ...
  it('calculates fallback velocity correctly when API velocity is missing', () => {
    ...
    const statCards = screen.getAllByText('Orbital Velocity');
    expect(statCards.length).toBeGreaterThan(0);
    
    // Try to find the calculated value
    expect(screen.getByText(expectedVelocityRegex)).toBeInTheDocument();
  });
});
